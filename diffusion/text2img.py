...
def run_torch_text2img(parser, args, output_path):
    import torch
...
def run_ort_text2img(parser, args, output_path):
    ...
    generator = np.random.RandomState(args.seed) if args.seed != 0 else None
...
def main():
    ap = argparse.ArgumentParser(description="Stable Diffusion text2img using PyTorch DirectML by default, or ONNX Runtime with --ort_dir")
    ...
    if args.ort_dir:
        run_ort_text2img(ap, args, output_path)
    else:
        run_torch_text2img(ap, args, output_path)
